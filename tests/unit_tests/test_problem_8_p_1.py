...
class TestProblem8P1(object):
    ...
    def test_push(self, push_vals, expected_stack):
        ...
        for val in push_vals:
            stack.push(val)
        while stack.stack:
            assert stack.pop().val == expected_stack.pop()
    ...
    def test_get_max(self, stack_vals, max_val):
        stack = self.instantiate_solution().StackWithMax(stack_vals)
        assert stack.get_max() == max_val
    ...
    def test_get_max_with_push(self, stack_vals, push_vals, max_val):
        stack = self.instantiate_solution().StackWithMax(stack_vals)
        for val in push_vals:
            stack.push(val)
        assert stack.get_max() == max_val
    ...
    def test_get_max_with_push(self, stack_vals, pop_count, max_val):
        stack = self.instantiate_solution().StackWithMax(stack_vals)
